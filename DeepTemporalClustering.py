...
from datasets import load_Benign_data,load_Cancer_data
...
class DTC:
    ...
    def pretrain(self, X,
        ...
        callbacks_list = []
        model_checkpoint = tf.keras.callbacks.ModelCheckpoint(os.path.join(save_dir, 'AE_checkpoint.hdf5'),
                                                                  monitor='val_loss',
                                                                  save_best_only=True)
        ...
        # Begin pretraining
        t0 = time()
        history=self.autoencoder.fit(X, X, batch_size=batch_size, epochs=epochs, verbose=verbose, validation_split=0.3)
        ...
        self.pretrained = True
        self.encoder.save_weights('{}/e_weights-epoch{}.h5'.format(save_dir, epochs))
        print('Pretrained encoder weights are saved to {}/e_weights-epoch{}.h5'.format(save_dir, epochs))
        self.decoder.save_weights('{}/d_weights-epoch{}.h5'.format(save_dir, epochs))
        print('Pretrained decoder weights are saved to {}/d_weights-epoch{}.h5'.format(save_dir, epochs))
...
#    cudaEnvironGPUCode =1
#
#
#    os.environ["CUDA _VISIBLE_DEVICES"] = str (cudaEnvironGPUCode)
...
#
    ...
    # Load data
    data_dir="Z:\\shared\\images\\ProstateVGH-2\\Data\\Dataset\\"
    #(X_train, y_train), (X_val, y_val) = load_data(args.dataset), (None, None)  # no train/validation split for now
    (X_train_b, X_val_b) = load_Benign_data(data_dir+"RF_allProstate_Benign.hdf5")
    (X_train_c, X_val_c) = load_Cancer_data(data_dir+"RF_allProstate_Cancer.hdf5")

    X_train=np.concatenate((X_train_b,X_train_c),axis=0)
    del X_train_b
    del X_val_b
    del X_train_c
    del X_val_c
    ...
    history=dtc.pretrain(X=X_train, optimizer=pretrain_optimizer,
    ...
    #loss_val=np.concatenate(loss_val,axis=0)
    plt.plot(loss_train)
    #plt.plot(loss_val)
